...
function setup() {
  ...
  invisibleGround = createSprite(width/2,height - 295,width,10);
...
function draw() {
  ...
  if (gameState===PLAY){
    ...
    if(keyWentDown(32) && trex.y >= 159 || touches.length > 0) {
      trex.velocityY = -12;
      touches = [];
    }

    if(keyWentUp(32) && trex.y > 2)
    {
...
function spawnObstacles() {
  if(frameCount % 60 === 0) {
    var obstacle = createSprite(width,height - 320,10,40);
    //obstacle.debug = true;
